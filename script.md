...
IdVeiculoLocacao int not null,
constraint fkidLocacao
foreign key (idLocacao)
references locacao(idLocacao),
constraint fkIdVeiculoLocacao
foreign key (IdVeiculoLocacao)
references veiculo(idVeiculo)
);
...
insert into LocacaoVeiculo (idLocacao, idVeiculoLocacao) values
     (1, 2),
     (2, 3),
     (3, 5),
     (4, 8),
...
-- Atividade4:
select LocacaoVeiculo.idVeiculoLocacao, veiculo.modelo, veiculo.marca,
count(*) qtd_locacoes
from veiculo
inner join LocacaoVeiculo
on veiculo.idveiculo = LocacaoVeiculo.idVeiculoLocacao
group by idVeiculoLocacao
order by qtd_locacoes desc;

-- Atividade5:

select cliente.nome as Nome_Devedor, 
pagamento.valorTotal as Valor_Total_a_Pagar, 
pagamento.estado as Estado_Pagamento
from locacao
inner join cliente
...
group by Nome_Devedor, Valor_Total_a_Pagar, Estado_Pagamento
